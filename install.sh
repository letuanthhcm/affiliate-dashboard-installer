...
if [ "$RESTART_WEB" -eq 1 ]; then
  ...
  selection="$(PM2_JSON="$before" APP_DIR="$APP_DIR" PM2_APP="$PM2_APP" node -e "const a=JSON.parse(process.env.PM2_JSON);const x=a.filter(p=>p.pm2_env&&p.pm2_env.pm_cwd===process.env.APP_DIR&&!/cron/i.test((p.name||'')+' '+(p.pm2_env.pm_exec_path||''))&&(!process.env.PM2_APP||p.name===process.env.PM2_APP));if(x.length!==1)process.exit(2);process.stdout.write(x[0].name)")" || fail WEB_PM2_PROCESS_NOT_UNIQUE
