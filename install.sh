...
SOURCE_ROLLBACK_RESULT=NOT_REQUIRED; LOCKFILE_ROLLBACK_RESULT=NOT_REQUIRED; CREATED_FILES_ROLLBACK_RESULT=NOT_REQUIRED; NODE_MODULES_ROLLBACK_RESULT=NOT_REQUIRED; RUNTIME_ROLLBACK_RESULT=NOT_REQUIRED; OVERALL_ROLLBACK_RESULT=NOT_REQUIRED; NODE_MODULES_ORIGINAL_STATE=UNKNOWN
...
ORIGINAL_HEALTH_TARGET=NONE; ORIGINAL_HEALTH_HTTP=NONE; ORIGINAL_HEALTH_BODY_HASH=NONE; RESTORED_HEALTH_HTTP=NONE; RESTORED_HEALTH_BODY_HASH=NONE
...
clean() { printf '%s' "$1" | tr '\r\n=' '___'; }
summary() { for key in INSTALLER_STATUS MODE APP_DIR IS_GIT_WORKTREE GIT_TOPLEVEL CONSUMER_SHA CONSUMER_BRANCH PACKAGE_VERSION PACKAGE_SHA REQUESTED_PACKAGE_SHA REQUESTED_PACKAGE_VERSION PREEXISTING_NODE_MODULES_VERSION INSTALLED_PACKAGE_VERSION INSTALLED_PACKAGE_COMMIT PACKAGE_MANAGER LOCKFILE LOCKFILE_TRACKED LOCKFILE_VALIDATION PRECHECK SUPPORTED_CONSUMER LEGACY_CONSUMER MIGRATION_REQUIRED MIGRATION_ELIGIBLE COMPATIBILITY_REASON LEGACY_STRATEGY DASHBOARD_LOCALS_VARIANT DIRTY_TARGET_PATHS DIRTY_NON_TARGET_PATHS DIRTY_POLICY_RESULT BACKUP_DIR BACKUP_MANIFEST ROLLBACK_AVAILABLE ROLLBACK_RESULT SOURCE_ROLLBACK_RESULT LOCKFILE_ROLLBACK_RESULT CREATED_FILES_ROLLBACK_RESULT NODE_MODULES_ROLLBACK_RESULT RUNTIME_ROLLBACK_RESULT OVERALL_ROLLBACK_RESULT NODE_MODULES_ORIGINAL_STATE MIGRATION_PLAN MIGRATION_RESULT INSTALL_RESULT PACKAGE_VERIFY TARGETED_PACKAGE_TESTS TARGETED_CONSUMER_TESTS PRODUCTION_LIKE_RENDER NO_DUPLICATE_RENDER IDEMPOTENCY_TEST PROTECTED_PATHS PROTECTED_PATHS_PRESERVED WEB_PM2_PROCESS WEB_RESTARTED WEB_PID_BEFORE WEB_PID_AFTER WEB_RESTART_COUNT_BEFORE WEB_RESTART_COUNT_AFTER CRON_PM2_PROCESSES CRON_RESTARTED CRON_RESTART_COUNT_BEFORE CRON_RESTART_COUNT_AFTER PM2_ONLINE_RESULT LISTENER_RESULT LISTENER_PORT LISTENER_ATTEMPTS LISTENER_READY_AFTER_SECONDS HEALTH_TARGET HEALTH_TARGET_SOURCE HEALTH_TARGET_RESOLUTION HEALTH_PRIMARY_TARGET HEALTH_PRIMARY_HTTP HEALTH_PRIMARY_LAST_HTTP HEALTH_PRIMARY_RESULT HEALTH_FALLBACK_TARGET HEALTH_FALLBACK_HTTP HEALTH_FALLBACK_LAST_HTTP HEALTH_FALLBACK_RESULT HEALTH_ATTEMPTS HEALTH_READY_AFTER_SECONDS HEALTH_TIMEOUT_SECONDS HEALTH_CHECK ORIGINAL_HEALTH_TARGET ORIGINAL_HEALTH_HTTP ORIGINAL_HEALTH_BODY_HASH RESTORED_HEALTH_HTTP RESTORED_HEALTH_BODY_HASH FINAL_RESULT; do eval "v=\${$key}"; printf '%s=%s\n' "$key" "$(clean "$v")"; done; }
rollback_runtime() {
  [ "$WEB_RESTARTED" = YES ] || return 0
  RUNTIME_ROLLBACK_RESULT=FAIL
  pm2 restart "$WEB_PM2_PROCESS" >/dev/null 2>&1 || return 0
  rollback_deadline=$(($(date +%s) + HEALTH_TIMEOUT_SECONDS))
  while :; do
    rollback_state="$(pm2 jlist 2>/dev/null || printf '[]')"
    rollback_online="$(PM2_JSON="$rollback_state" NAME="$WEB_PM2_PROCESS" node -e "const p=JSON.parse(process.env.PM2_JSON).find(x=>x.name===process.env.NAME);process.stdout.write(p&&p.pm2_env&&p.pm2_env.status==='online'?'YES':'NO')" 2>/dev/null || printf NO)"
    if [ "$rollback_online" = YES ]; then
      RESTORED_HEALTH_HTTP="$(curl -sS -o "$BACKUP_DIR/restored-health.body" -w '%{http_code}' --connect-timeout 1 --max-time 2 "$ORIGINAL_HEALTH_TARGET" 2>/dev/null || true)"
      if [ -n "$RESTORED_HEALTH_HTTP" ] && [ -f "$BACKUP_DIR/restored-health.body" ]; then
        RESTORED_HEALTH_BODY_HASH="$(git hash-object --no-filters "$BACKUP_DIR/restored-health.body" 2>/dev/null || printf HASH_FAILED)"
        if [ "$RESTORED_HEALTH_HTTP" = "$ORIGINAL_HEALTH_HTTP" ] && [ "$RESTORED_HEALTH_BODY_HASH" = "$ORIGINAL_HEALTH_BODY_HASH" ]; then RUNTIME_ROLLBACK_RESULT=PASS; return 0; fi
      fi
    fi
    [ "$(date +%s)" -lt "$rollback_deadline" ] || return 0
...
rollback() {
  ...
  for result in "$SOURCE_ROLLBACK_RESULT" "$LOCKFILE_ROLLBACK_RESULT" "$CREATED_FILES_ROLLBACK_RESULT" "$NODE_MODULES_ROLLBACK_RESULT" "$RUNTIME_ROLLBACK_RESULT"; do [ "$result" = PASS ] || [ "$result" = NOT_REQUIRED ] || OVERALL_ROLLBACK_RESULT=FAIL; done
...
if [ "$LEGACY_CONSUMER" = YES ]; then
  required='modules/app/helpers/setAppRoutes.js modules/app/helpers/node_cache_factory.js modules/dashboard/controllers/dashboard.admin.js themes/admin/dashboard/dashboard-admin.pug'
...
NODE
  ...
  HEALTH_TARGET="$HEALTH_PRIMARY_TARGET"; HEALTH_ROOT_TARGET="http://127.0.0.1:$health_port/"; HEALTH_FALLBACK_TARGET=NOT_REQUIRED; LISTENER_PORT="$health_port"
  ORIGINAL_HEALTH_TARGET="http://127.0.0.1:$health_port/health"
  ORIGINAL_HEALTH_HTTP="$(curl -sS -o "$BACKUP_DIR/original-health.body" -w '%{http_code}' --connect-timeout 1 --max-time 2 "$ORIGINAL_HEALTH_TARGET" 2>/dev/null || true)"
  [ -n "$ORIGINAL_HEALTH_HTTP" ] && [ "$ORIGINAL_HEALTH_HTTP" != 000 ] && [ -f "$BACKUP_DIR/original-health.body" ] || fail ORIGINAL_HEALTH_SNAPSHOT_FAILED
  ORIGINAL_HEALTH_BODY_HASH="$(git hash-object --no-filters "$BACKUP_DIR/original-health.body" 2>/dev/null)" || fail ORIGINAL_HEALTH_SNAPSHOT_FAILED
