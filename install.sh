...
WEB_PM2_PROCESS=NONE; WEB_RESTARTED=NO; CRON_PM2_PROCESSES=NONE; CRON_RESTARTED=NO; HEALTH_TARGET=NONE; HEALTH_TARGET_SOURCE=NONE; HEALTH_TARGET_RESOLUTION=NOT_RUN; HEALTH_CHECK=NOT_RUN; MUTATED=0
...
if [ "$RESTART_WEB" -eq 1 ]; then
  ...
  if [ -n "$HEALTH_URL" ]; then HEALTH_TARGET="$HEALTH_URL"; HEALTH_TARGET_SOURCE=EXPLICIT_URL; HEALTH_TARGET_RESOLUTION=PASS; else
...
NODE
    ...
    HEALTH_TARGET_SOURCE="${health_resolution%%|*}"; health_port="${health_resolution#*|}"; HEALTH_TARGET="http://127.0.0.1:$health_port/health"; HEALTH_TARGET_RESOLUTION=PASS
    if ! curl -fsS "$HEALTH_TARGET" >/dev/null 2>&1; then HEALTH_TARGET="http://127.0.0.1:$health_port/"; fi
  fi
