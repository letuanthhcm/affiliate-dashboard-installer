...
exit 0
YARN
  chmod +x "$r/fake-bin/yarn"; rollback_files='package.json yarn.lock modules/app/helpers/setAppRoutes.js modules/dashboard/controllers/dashboard.admin.js themes/admin/dashboard/dashboard-admin.pug'; before_manifest=''; for f in $rollback_files; do before_manifest="$before_manifest$f:$(sha256sum "$r/$f" | awk '{print $1}');"; done; out=$(cd "$r" && PATH="$r/fake-bin:$PATH" "$INSTALLER" --package-sha "$SHA" --migrate-legacy 2>&1 || true); grep -q '^FINAL_RESULT=PACKAGE_VERIFY_FAILED$' <<<"$out"; grep -q '^ROLLBACK_AVAILABLE=YES$' <<<"$out"; grep -q '^ROLLBACK_RESULT=PASS$' <<<"$out"; grep -q '^INSTALL_RESULT=ROLLED_BACK$' <<<"$out"; grep -q '^WEB_RESTARTED=NO$' <<<"$out"; [ ! -e "$r/config/analytics-package.js" ]; after_manifest=''; for f in $rollback_files; do after_manifest="$after_manifest$f:$(sha256sum "$r/$f" | awk '{print $1}');"; done; [ "$before_manifest" = "$after_manifest" ]; git -C "$r" diff --quiet; rm -rf "$r"
